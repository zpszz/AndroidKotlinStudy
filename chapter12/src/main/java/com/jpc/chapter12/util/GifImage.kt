package com.jpc.chapter12.util
...
class GifImage {
    ...
    class GifFrame(// to access image & delay w/o interface
    ...
    var width: Int = 0 // full image width
        private set
    var heigh: Int = 0 // full image height
        private set
    ...
    private var frames: Vector<GifFrame>? = null // frames read from current file
    ...
    private fun setPixels() {
        ...
        image = Bitmap.createBitmap(dest, width, heigh, Bitmap.Config.RGB_565)
    }

    private fun getFrame(n: Int): Bitmap? {
        var im: Bitmap? = null
        if ((n >= 0) && (n < frameCount)) {
    ...
    fun getFrames(): Array<GifFrame>? {
        if (null != frames) return frames!!.toTypedArray<GifFrame>()
        return null
    }
    ...
    fun nextBitmap(): Bitmap {
        ...
        return (frames!!.elementAt(frameindex) as GifFrame).image
    }

    fun nextDelay(): Int {
        return (frames!!.elementAt(frameindex) as GifFrame).delay
    }
    ...
    private fun readHeader() {
        ...
        if (!id.uppercase(Locale.getDefault()).startsWith("GIF")) {
            status = STATUS_FORMAT_ERROR
            return
        }
        readLSD()
    ...
    private fun readImage() {
        ...
        image = Bitmap.createBitmap(width, heigh, Bitmap.Config.RGB_565)
